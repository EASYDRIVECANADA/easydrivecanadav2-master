...
export const applyFleetPriceMarkup = (price) =>
  Number(price || 0) + FLEET_PRICE_MARKUP;
...
export function parseFleetInventoryRows(rows) {
  ...
  rows.slice(1).forEach((row, offset) => {
    ...
    if (seenStock.has(stock)) {
      skipped.push({
        row: sourceRow,
        reason: `Duplicate stock number ${stock} in workbook`,
      });
      return;
    }
    ...
    if (seenVin.has(vin)) {
      skipped.push({
        row: sourceRow,
        reason: `Duplicate VIN ${vin} in workbook`,
      });
      return;
    }
